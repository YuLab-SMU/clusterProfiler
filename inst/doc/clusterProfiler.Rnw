...
% \VignetteDepends{AnnotationDbi, GO.db, org.Hs.eg.db, GOstats, ggplot2, plyr, methods}
...
@
\captionof{figure}{Example of comparing MF ontology distribution using dotplot.}
...
\begin{center}
<<fig=TRUE, width=8>>=
print(plot(xx, type="bar", limit=NULL, by="count", caption="MF Ontology Distribution Comparison"))
@
\captionof{figure}{Example of comparing MF ontology distribution using barplot.}
\label{Fig:compareClusterPlot-groupGO-2}
\end{center}
\end{figure}
...
@
\captionof{figure}{Example of comparing CC ontology enrichment among gene clusters.}
...
@
\captionof{figure}{Example of comparing KEGG enrichment among gene clusters.}
...
<<KEGG>>=
yy <- enrichKEGG(gcSample[[5]], organism="human", pvalueCutoff=0.01)
head(summary(yy))
@
...
\begin{center}
<<fig=TRUE>>=
print(plot(yy, caption="BP ontology categrories at level 2"))
@
\captionof{figure}{Example of annotating gene list to BP ontology at level 2.}
\label{Fig:groupGOPlot}
\end{center}
\end{figure}
