...
% \VignetteDepends{AnnotationDbi, GO.db, org.Hs.eg.db, ggplot2, plyr, methods, qvalue, KEGG.db}
...
By default, only top 5 categories of each cluster was plotted. User can changes the parameter \textit{showCategory} to specify how many categories of each cluster to be plotted, and if \textit{showCateogry} was set to NULL, the whole result will be plotted. By default, the dot sizes were based on their corresponding row percentage, and user can set the parameter \textit{by} to "count" to make the comparison based on gene counts. 
