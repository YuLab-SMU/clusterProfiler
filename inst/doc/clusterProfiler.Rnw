...
comparing biological themes among gene clusters. \textit{OMICS: A
  Journal of Integrative Biology}. 2012, 16(5), in press. \\
...
<<enrichGO>>=
y <- enrichGO(gene=gcSample[[2]], organism="human", ont="MF", pvalueCutoff=0.01, qvalueCutoff=0.05, readable=TRUE)
summary(y)
@
...
<<enrichKEGG>>=
z <- enrichKEGG(gene=gcSample[[3]], organism="human", pvalueCutoff=0.05, qvalueCutoff=0.05, readable=TRUE)
summary(z)
@
...
encourage user to use \Rfunction{enrichPathway} in \Rpackage{rPA}
\url{https://github.com/GuangchuangYu/rPA}, which use Reactome as a
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=.7\linewidth]{omics2012.eps}
...
\Rpackage{DOSE} and \Rpackage{rPA} and compare gene cluster in the
context of disease and reactome pathway as demonstrated in the online
vignette of \Rpackage{DOSE} and \Rpackage{rPA} respectively.
...
<<plot enrichGO, eval=F>>=
plot(y, titile="MF Enrichment analysis", showCategory=10)
@

\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=.7\linewidth]{enrichGO.eps}
    \caption{Example of plotting functional profiles}
    \label{Fig:enrichGO}
  \end{center}
\end{figure}
...
xx <- compareCluster(gcSample, fun=enrichGO, ont="CC", organism="human", pvalueCutoff=0.05, qvalueCutoff=0.05)
plot(xx)
@

\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=.7\linewidth]{compareCluster.eps}
...
(\Rfunction{enrichKEGG}) provided in this package, but also other
  ...
  \Rfunction{enrichPathway} in \Rpackage{rPA}). More details can be
  found in the vignettes of \Rpackage{DOSE} and \Rpackage{rPA}.
