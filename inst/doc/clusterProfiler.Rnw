...
After clustering analysis, researchers not only want to determine whether there is a common theme of a particular gene cluster, but also to compare the biological themes among gene clusters, which have different expression profiles.
...
\begin{center}
<<fig=TRUE, width=8>>=
print(plot(xx, type="dot", title="MF Ontology Distribution Comparison"))
@
...
<<compareCluster>>=
xx <- compareCluster(gcSample, fun=enrichGO, organism="human", ont="CC", pvalueCutoff=0.01)
head(summary(xx))
@
...
\begin{center}
<<fig=TRUE>>=
print(plot(xx, title="CC Ontology Enrichment Comparison"))
@
...
\begin{center}
<<fig=TRUE>>=
print(plot(xx, type="bar", by="count", title="CC Ontology Enrichment Comparison using bar chart"))
@
\caption{Example of comparing CC ontology enrichment among gene clusters.}
\label{Fig:compareClusterPlot-enrichGO-2}
\end{center}
\end{figure}
...
\begin{center}
<<fig=TRUE, width=7>>=
print(plot(xx, type="dot", title="KEGG Enrichment Comparison"))
@
...
\begin{center}
<<fig=TRUE>>=
print(plot(yy, title="KEGG Enrichment Analysis"))
@
